...
export const getSiteRelativePathAndFileNames = function (
  absoluteDirectory,
  files_
) {
  ...
  for (var i in files) {
    ...
    if (!qstr.contains(absolutePathAndFileName, "\\node_modules\\")) {
      absolutePathAndFileName = qstr.replaceAll(
      ...
      if (fs.statSync(absolutePathAndFileName).isDirectory()) {
        ...
        const relativePathAndFileName = qfil.getRelativePathAndFileName(
          absolutePathAndFileName
        );
        const fixedPathAndFileName = qfil.convertBackSlashesToForwardSlashes(
